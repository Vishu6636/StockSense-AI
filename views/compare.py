...
def page_compare():
    ...
    if st.button("Compare Now", key="cmp_go", type="primary", use_container_width=True):
        tickers = [stock_map[s] for s in [s1, s2, s3] if s and s in stock_map]
        if len(tickers) < 2:
            st.warning("Please select at least 2 different stocks to compare.")
            return
    ...
    if len(stocks) < 2:
        st.warning("Could not fetch complete data for at least 2 valid stocks. Please check your connection or selections.")
        return
    ...
    for label, fn in metrics:
        ...
        for i, s in enumerate(stocks):
            try: val = fn(s["info"])
            except (KeyError, IndexError, TypeError, ValueError, AttributeError): val = "N/A"
    ...
    st.markdown(f"""<div class="rec-banner {best[2]}"><div>
    ...
    for s, verdict, cls, icon, reasons in scores:
        ...
        with st.expander(f"{name} — {verdict}"):
