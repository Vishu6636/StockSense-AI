...
def page_watchlist():
    ...
    if not st.session_state.logged_in:
        ...
        if st.button("Login to Unlock", key="wl_login_btn", type="primary", use_container_width=True):
    ...
    if not wl:
        st.info("Your watchlist is currently empty. Add stocks by clicking the Watch button on any stock page.")
        ...
        if st.button("Find Stocks to Watch", key="wl_empty_btn", type="primary"):
    ...
    for item in enriched_wl:
        ...
        c1, c2, c3, c4 = st.columns([3, 2, 2, 2.5])
        with c1:
        ...
        with c4:
            ...
            with ck1:
                if st.button("Analyze", key=f"wl_go_{ticker}", help=f"Analyze {ticker}"):
                    ...
                    st.rerun()
            with ck2:
                if st.button("Remove", key=f"wl_rmbtn_{ticker}", help="Remove from watchlist"):
