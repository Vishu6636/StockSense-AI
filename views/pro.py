...
def reset_pro_state():
    pro_keys = [
        "pro_df", "pro_pe", "pro_de", "pro_roe", "pro_prom", "pro_npm",
        "pro_eps", "pro_fcf", "pro_adv", "pro_bud", "pro_q50", "pro_sel"
    ]
    for k in pro_keys:
...
def page_pro():
    if st.session_state.get("_active_view") != "pro":
        reset_pro_state()
        st.session_state._active_view = "pro"
    ...
    if st.button("Run Screener", key="pro_run", type="primary", use_container_width=True):
    ...
    if "pro_df" in st.session_state:
        ...
        if df.empty:
            st.warning("No stocks matched your exact criteria. Try relaxing your P/E, ROE, or Debt filters.")
            st.info("Tip: Try turning off 'Quick Scan 50' or selecting 'Any' for Economic Moat.")
            return
        passed = df[df["Status"].str.contains("Pass", na=False)]
        rejected = df[df["Status"].str.contains("Reject", na=False)]

        tab_pass, tab_rej, tab_chart = st.tabs([f":material/check_circle: Passed ({len(passed)})", f":material/cancel: Rejected ({len(rejected)})", ":material/bar_chart: Score Chart"])
        ...
        with tab_pass:
            ...
            for i, row in top10_passed.iterrows():
                ...
                with rc4:
                    is_pass = "Pass" if "Pass" in str(row["Status"]) else "Reject"
            ...
            if not passed.empty:
                sel = st.selectbox("Select stock to analyze:", passed["Name"].tolist(), key="pro_sel")
                sel_ticker = passed[passed["Name"] == sel]["Ticker"].values[0]
                if st.button(f"Analyze {sel}", key="pro_an"):
